...
export default function MergePage() {
    ...
    // ── Merge-specific state (from original App.jsx) ──
    ...
    // ── Process (exact merge logic from original App.jsx) ──
    const handleProcessAll = async () => {
        if (items.length < 2) {
            setErrorMessage('⚠️ Please select at least 2 images to merge.');
            return;
        }
        setIsProcessing(true);
        try {
            const filesToMerge = items.map(item => item.file);
            const result = await mergeImages(filesToMerge, direction, padding, bgColor);
            const merged = [{ ...items[0], status: 'done', processedResult: result }];
            setItems(merged);
            setPreviewIndex(0);
        } catch (err) {
            console.error(err);
            setErrorMessage('Failed to merge images. Please try again.');
        } finally {
    ...
    const doDownloadSingle = (item) => {
        ...
        a.download = item.processedResult.file?.name || 'merged_image.png';
        a.click();
    };
    ...
    const handleDownloadSingle = (item) => {
        ...
        if (isPro || isAdFree) {
            ...
            setAdModal({
                ...
                fileName: item.processedResult.file?.name || 'merged_image.png',
            });
        }
    };
        ...
        >
            ...
            <div className="page-title-area" style={{ marginBottom: 36, textAlign: 'center' }}>
                <h1 style={{
                    ...
                    <span className="grad-text">Merge Images</span>
                </h1>
                            ...
                            <div style={{
                                ...
                                {isProcessing && (
                                    <div style={{
                                        ...
                                        <span style={{ fontSize: '0.85rem', fontWeight: 700, color: '#c7d2fe' }}>Merging…</span>
                                    </div>
                                )}
                        ...
                        <div className="animate-in">
                            <div style={{
                                ...
                                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                                    ...
                                    {processedCount > 0 && (
                                        <span className="badge badge-emerald">{processedCount} merged</span>
                                    )}
                                </div>
                ...
                {items.length > 0 && (
                    <div className="settings-panel" style={{ position: 'sticky', top: 90, display: 'flex', flexDirection: 'column', gap: 16 }}>
                        <div style={{
                            ...
                            {/* Merge controls (exact props from original App.jsx) */}
                            <MergeControls
                                direction={direction}
