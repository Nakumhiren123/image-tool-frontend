...
import { applyFilterToImage, downloadZip, loadImage } from '../lib/imageEngine';
import { Trash2, ImageIcon } from 'lucide-react';

// CSS filter map — exact same as original App.jsx preview panel
const FILTER_CSS = {
    ...
    contrast: 'contrast(1.5) brightness(1.05)',
...
export default function FilterPage() {
    ...
    // ── Filter-specific state (from original App.jsx) ──
    ...
    // ── Process (exact filter logic from original App.jsx) ──
    const handleProcessAll = async () => {
        ...
        try {
            ...
            for (const item of items) {
                const result = await applyFilterToImage(item.file, filterMode);
    ...
    const doDownloadZip = () => {
        ...
        if (valid.length) downloadZip(valid, 'piccraft_filter_batch.zip');
    };
    ...
    const handleDownloadZip = () => {
        if (isPro || isAdFree) {
            ...
            setAdModal({
                ...
                fileName: 'piccraft_filter_batch.zip',
            });
        }
    };
    ...
    // CSS filter to show live preview before processing (from original App.jsx)
    const liveFilterCss = filterMode !== 'normal' ? (FILTER_CSS[filterMode] || 'none') : 'none';
        ...
        >
            ...
            <div className="page-title-area" style={{ marginBottom: 36, textAlign: 'center' }}>
                <h1 style={{
                    ...
                    <span className="grad-text">Photo Filters & Color Effects</span>
                </h1>
                <p style={{ fontSize: '0.95rem', color: 'var(--text-3)', maxWidth: 500, margin: '0 auto' }}>
                    Apply Black &amp; White, Sepia Vintage, Invert, Warm Sunset, Cool Cyan, and Cyber Neon filters
                </p>
            </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
                    ...
                    {items.length > 0 && firstItem && (
                        <div style={{
                            ...
                            {/* Image area — live CSS filter preview (exact from original App.jsx) */}
                            <div style={{
                                ...
                                <img
                                    ...
                                    style={{
                                        maxWidth: '100%', maxHeight: 420, objectFit: 'contain', display: 'block', borderRadius: 4,
                                        // Apply live CSS filter preview when no processed result yet
                                        filter: firstItem.processedResult ? 'none' : liveFilterCss,
                ...
                {items.length > 0 && (
                    <div className="settings-panel" style={{ position: 'sticky', top: 90, display: 'flex', flexDirection: 'column', gap: 16 }}>
                        <div style={{
                            ...
                            {/* Filter controls (exact props from original App.jsx) */}
                            <FilterControls
