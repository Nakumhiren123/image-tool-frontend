...
export default function ConverterPage({ from, to }) {
    ...
    const handleFilesSelected = async (newFiles) => {
        ...
        const MAX_UPLOAD_MB = isPro ? 50 : 10;
        ...
        const filesWithinSizeLimit = newFiles.filter(
            file => file.size <= MAX_UPLOAD_BYTES
        );
        ...
        if (oversizedFiles.length > 0) {
            ...
            setErrorMessage(
                `⚠️ Maximum file size is ${MAX_UPLOAD_MB} MB. Rejected: ${names}`
            );
        }

        const validFiles = filesWithinSizeLimit.filter(f =>
        ...
        // Strict source-format validation.
        // Example: JPG → PNG accepts only .jpg files.
        // JPEG → PNG accepts only .jpeg files.
        if (from) {
            const requiredExtension = from.toLowerCase();

            const wrongFormatFiles = validFiles.filter((file) => {
                const extension = file.name
                    .split('.')
                    .pop()
                    ?.toLowerCase();

                return extension !== requiredExtension;
            });

            if (wrongFormatFiles.length > 0) {
                setErrorMessage(
                    `Invalid image file type. This converter accepts only ${from.toUpperCase()} files.`
                );
